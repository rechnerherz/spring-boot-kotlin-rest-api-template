 ...
 *
 * The order is set to run before the transaction advice defined in [at.rechnerherz.example.config.JpaConfig].
 *
...
class ProfilingAspect(
    ...
    @Pointcut(
        "within(@at.rechnerherz.example.config.aop.NoProfiling *)" +
                " || @annotation(at.rechnerherz.example.config.aop.NoProfiling)"
    )
    fun noProfiling() {
    }
